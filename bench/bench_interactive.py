...
class InteractiveBenchmark(BenchmarkFramework):
    def __init__(self, benchresults_cls=BenchResults, cli_args=None):
        super().__init__("interactive_benchmark", benchresults_cls, cli_args)
    ...
    def generate_configs(self, configs: List[Dict]) -> List[Dict]:
        ...
        configs = add_config_option("benchmark", ["interactive"], configs)
        configs = add_config_option(
...
def main():
    ...
    interactive_bench = InteractiveBenchmark()
    # Check that trace data dir exists
    if not os.path.exists(interactive_bench.args.data_dir):
        raise Exception(
            "Interactive data directory not found: %s" % interactive_bench.args.data_dir
        )
    log.info("Interactive data directory: %s", interactive_bench.args.data_dir)
    interactive_bench.benchmark()
